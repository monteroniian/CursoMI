...
class AppState {
    constructor() {
        const raw = JSON.parse(localStorage.getItem('cursosmi_user') || 'null');
        this.currentUser = (raw && raw.user) ? { ...raw.user, sessionToken: raw.sessionToken } : raw;
        this.searchQuery = '';
    }

    setUser(user) {
        const normalized = (user && user.user) ? { ...user.user, sessionToken: user.sessionToken } : user;
        this.currentUser = normalized;
        if (normalized) {
            localStorage.setItem('cursosmi_user', JSON.stringify(normalized));
        } else {
