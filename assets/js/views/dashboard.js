...
export async function renderDashboard(containerElement, rawUser) {
    const currentUser = (rawUser && rawUser.user) ? { ...rawUser.user, sessionToken: rawUser.sessionToken } : rawUser;
...
function renderAuthForms(container) {
    ...
    formLogin?.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (res && !res.error) {
                const userData = res.user ? { ...res.user, sessionToken: res.sessionToken } : res;
                localStorage.setItem('cursosmi_user', JSON.stringify(userData));
    ...
    formRegister?.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (res && !res.error) {
                const userData = res.user ? { ...res.user, sessionToken: res.sessionToken } : res;
                localStorage.setItem('cursosmi_user', JSON.stringify(userData));
                showToast(`¡Cuenta creada exitosamente! Bienvenido, ${userData.nombre}.`, 'success');
                window.location.reload();
            } else {
