...
const app = express();
app.set('trust proxy', 1); // Confiar en el reverse proxy de Render para detectar HTTPS
...
app.post('/api/auth/login', (req, res) => {
    const loginKey = req.body.loginKey || req.body.emailOrDni || req.body.email;
    ...
    if (loginKey.trim() === 'ADMIN' && password === '123456789123') {
...
app.post('/api/payments/mercadopago/create-preference', async (req, res) => {
    ...
    db.get(`SELECT * FROM courses WHERE id = ?`, [courseId], async (err, course) => {
        ...
        db.get(`SELECT * FROM users WHERE id = ?`, [validUserId], async (err, user) => {
            ...
            let returnUrl = `${baseUrl}/payment-return?course_id=${course.id}&user_id=${validUserId}`;
            
            // Forzar HTTPS en producción para que Mercado Pago acepte auto_return
            if (!isLocalhost && returnUrl.startsWith('http://')) {
            ...
            // auto_return solo está permitido en Mercado Pago para URLs HTTPS
